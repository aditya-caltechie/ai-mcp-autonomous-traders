...
## Two main players per trader (mindset)
...
### Quick: How to read this project

```
  Setup                    Runtime                     Output
  ─────                    ───────                     ──────
  [1] database      →      [10] trading_floor   →     [11] app
  [2] market              runs [9] traders            (dashboard)
  [3] accounts             every N min
  [4] reset.py
  seeds 4 traders
```

**Flow:** `reset.py` (4) seeds DB → `trading_floor.py` (10) runs `traders.py` (9) → each trader starts MCP servers (5,6), builds agents, runs LLM loop → `app.py` (11) reads DB and shows results.
...
### 1.1 Layer List (numbered for flow)

| # | Layer | File(s) | Role |
|---|-------|---------|------|
| 1 | database | `database.py` | SQLite schema & persistence |
| 2 | market | `market.py` | Market data (Polygon API + DB cache) |
| 3 | accounts | `accounts.py` | Account model & trading logic |
| 4 | reset | `reset.py` | Seed trader accounts & strategies |
| 5 | MCP servers | `accounts_server`, `push_server`, `market_server` | Expose tools/resources |
| 6 | mcp_params | `mcp_params.py` | MCP server configuration |
| 7 | accounts_client | `accounts_client.py` | Client to read from accounts MCP |
| 8 | templates/util/tracers | `templates.py`, `util.py`, `tracers.py` | Prompts, UI, tracing |
| 9 | traders | `traders.py` | Trader agent orchestration |
| 10 | trading_floor | `trading_floor.py` | Scheduler that runs all traders |
| 11 | app | `app.py` | Gradio dashboard (reads DB directly) |

### 1.2 Layered Block Diagram (build order, top → bottom)

```
┌─────────────────────────────────────────────────────────────────────────┐
│  11  app.py                                                             │
│      Gradio dashboard (reads DB directly)                               │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  10  trading_floor.py                                                   │
│      Scheduler: runs all traders every N minutes                        │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  9   traders.py                                                         │
│      Trader agent orchestration (MCP servers → agents → Runner)         │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                    ┌───────────────┼───────────────┐
                    ▼               ▼               ▼
┌──────────────┐ ┌──────────────┐ ┌──────────────────────────────────────┐
│  8a templates │ │  8b util     │ │  8c tracers                         │
│  Prompts      │ │  CSS, Color  │ │  Trace IDs, LogTracer → write_log   │
└──────────────┘ └──────────────┘ └──────────────────────────────────────┘
                    │               │
                    └───────┬───────┘
                            │
                    ...
                    ▼               ▼
┌────────────────────────────┐ ┌────────────────────────────────────────┐
│  7  accounts_client.py      │ │  6  mcp_params.py                     │
│  read_accounts_resource,   │ │  trader_mcp_server_params,             │
│  read_strategy_resource    │ │  researcher_mcp_server_params          │
└────────────────────────────┘ └────────────────────────────────────────┘
                    │               │
                    │       ┌───────┴───────┐
                    │       ▼               ▼
                    ...
                    │           ▼                   ▼
                    │   ┌──────────────┐   ┌─────────────────┐
                    │   │  2  market   │   │  1  database.py │
                    │   │  get_share_  │   │  accounts.db    │
                    │   │  price       │   │  write/read     │
                    └───│  Polygon     │───│  schema         │
                        └──────────────┘   └─────────────────┘
                                │
...
### 1.3 Dependency Flow (who uses whom)
...
```
        ...
        ▼                    ▼                    ▼
┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│  2 market.py │    │ 3 accounts   │    │ 8c tracers   │
└──────┬───────┘    └──────┬───────┘    └─────-─┬──────┘
       │                   │                    │
       │              ┌────┴─────┐              │
       │              ▼           ▼             │
...
## 3. Dependency Diagram (numbered)

Same flow as §1.3, with layer numbers in brackets:
...
```
              ...
              ▼                   ▼                   ▼
       ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
       │ [2] market   │  │ [3] accounts │  │ [8c] tracers │
       └──────┬───────┘  └──────┬─-─────┘  └──────┬─────-─┘
             │                  │                 │
             │            ┌────-┴────┐            │
             │            ▼          ▼             
             │     ┌──────────┐ ┌─────────────────────┐
             │     │ [4] reset│ │ [5] MCP servers     │
             │     └──────────┘ │ accounts, push, mkt │
             │                  └─────────┬────────-──┘
             │                            │
             │                  ┌─────────┴─────────┐
